# VS Code Devcontainer AI Assistant テンプレート集

本プロジェクトは、AI コーディングアシスタント（**Claude Code** や **Gemini CLI**）を、チーム共有の構成を汚さずに VS Code Devcontainer 環境へ導入するためのテンプレートリポジトリです。

## 🌟 プロジェクトの目的

開発チームで共有する `devcontainer.json` や `docker-compose.yml` には、プロジェクト共通の依存関係（言語、DB、ツールなど）のみを記述します。一方で、Claude Code や Gemini CLI などの AI ツールのインストールや認証情報の連携は、開発者個人のローカル環境に依存するため、以下の仕組みで分離・拡張できるように設計されています。
...
### 1. アシスタントの選択と配置
利用したいアシスタントのディレクトリ（`Claude Code/` または `Gemini CLI/`）内にある `.devcontainer/` の中身を、プロジェクトルートの `.devcontainer/` ディレクトリにコピーします。
...
### 3. 個人用設定の修正（個人設定）
`.devcontainer/docker-compose.local.yml` を開き、ホスト側にある AI ツールの設定ディレクトリ（認証情報が含まれる場所）のパスを、自分の PC の環境に合わせて修正します。
...
起動時に `local-init.sh` が自動的に実行され、AI ツールのインストールと認証情報のリンクが完了します。

## 🛠️ 注意事項
*   `docker-compose.local.yml` および `local-init.sh` は、個人固有の情報を扱うため、通常はプロジェクトの `.gitignore` に追加して管理外にすることを推奨します。
*   各アシスタントの詳細な設定方法については、それぞれのディレクトリ内にある `README.md` を参照してください。
