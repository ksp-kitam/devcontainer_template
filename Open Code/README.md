...
本テンプレートでは、AIコーディングアシスタント「[Open Code](https://opencode.ai/ja)」などの個人的な開発支援ツールは、

チームで共有する共通環境には含めない方針としています。

Open Codeのインストールや認証情報の連携は、開発者個人のローカル環境固有の設定（`local-init.sh` および `docker-compose.local.yml`）でのみ行い、
...
.devcontainer/
 ...
 ├── docker-compose.local.yml # 個人環境用の構成（Open Code設定等 / ※Git管理外）
 └── local-init.sh            # 個人環境用の初期化スクリプト（Open Codeインストール等 / ※Git管理外）
```
...
services:
  app:
    volumes:
      # マウント元(ローカル側)を Open Code の管理情報の保存場所に書き換える
      # コンテナ側の<ホームディレクトリ>/.claudeに設定される。
      - <your pc claude code data folder>:/opt/opencode_settings:cached
```
...
### Open Code の利用（設定済みの場合）

- local-init.sh でのセットアップが完了している場合、ターミナルからすぐにOpen Codeを呼び出せます。

- コンテナ内のターミナルを開きます。

- 以下のコマンドを実行してOpen Codeを起動します。
  ```bash
  opencode
  ```
